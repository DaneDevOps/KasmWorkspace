    ...
    then
        ...
        FILE=${KASM_INSTALL_BASE}/conf/database/seed_data/default_connection_proxies.yaml
	      if [ "${PUBLIC_HOSTNAME}" == "false" ] && [ -f "${FILE}" ]; then
            ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.connection_proxies.[])  .server_port = "'${DEFAULT_PROXY_LISTENING_PORT}'"' ${FILE}
	      fi
...
function set_database_ssl() {
    ...
    if [ "${DATABASE_HOSTNAME}" == "false" ] ; then
    	openssl req -x509 -nodes -days 1825 -newkey rsa:2048 -keyout ${KASM_INSTALL_BASE}/certs/db_server.key -out ${KASM_INSTALL_BASE}/certs/db_server.crt -subj "/C=US/ST=VA/L=None/O=None/OU=DoFu/CN=$(hostname)/emailAddress=none@none.none" 2> /dev/null
        ...
        if ! id -nu 70 > /dev/null 2>&1; then
		    if ! grep -q '^kasm_db:' /etc/passwd ; then
			    adduser -u 70 kasm_db &> /dev/null
            fi
		fi
		chown 70:70 ${KASM_INSTALL_BASE}/certs/db_server.key
        chown 70:70 ${KASM_INSTALL_BASE}/certs/db_server.crt
        chmod 0600 ${KASM_INSTALL_BASE}/certs/db_server.crt
        chmod 0600 ${KASM_INSTALL_BASE}/certs/db_server.key
    fi
}
...
function set_guac_token() {
    ...
    then
        ...
        KASM_GUAC_ID=$(cat /proc/sys/kernel/random/uuid)
        ...
        sed -i "s/00000000-0000-0000-0000-000000000000/${KASM_GUAC_ID}/g" ${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
        sed -i "s/00000000-0000-0000-0000-000000000000/${KASM_GUAC_ID}/g" ${KASM_INSTALL_BASE}/conf/database/seed_data/default_connection_proxies.yaml
        sed -i "s/SERVER_ADDRESS/kasm_guac/g" ${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
        sed -i "s/SERVER_PORT/${DEFAULT_PROXY_LISTENING_PORT}/g" ${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
        sed -i "s/ZONE/${SERVER_ZONE}/g" ${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
        sed -i "s/registration_token: REGISTRATION_TOKEN//g" ${KASM_INSTALL_BASE}/conf/app/kasmguac.app.config.yaml
    fi
...
function pull_images() {
    if [ ${OFFLINE_INSTALL} == "false" ] ; then
        ...
        docker exec kasm_db psql -U ${DATABASE_USER} -d ${DATABASE_NAME} -t  -c "SELECT name FROM images WHERE enabled = true;" | xargs -L 1 docker pull
    fi
}
...
function create_docker_network() {
    ...
    set +e
    docker network inspect ${kasm_network} &> /dev/null
    ret=$?
    set -e
    if [ $ret -ne 0 ]; then
        echo "Creating docker network ${kasm_network}"
        docker network create --driver=bridge kasm_default_network
    else
...
function display_help() {
    ...
    echo -e "| ${CMD}-l|--guac-token${NC}          | Set default guac token                                           |"
...
for index in "${!ARGSAPPEND[@]}"; do
  case ${ARGSAPPEND[index]} in
    ...
    -y|--default-registry-url)
        ...
        ;;
    -l|--guac-token)
        DEFAULT_GUAC_TOKEN="${ARGSAPPEND[index+1]}"
        ;;
    --guac-cluster-size)
...
if [ ${OFFLINE_INSTALL} == "false" ] || [ ${FORCE_DEPS_INSTALL} == "true" ]; then
    bash ${KASM_RELEASE}/install_dependencies.sh ${SKIP_V4L2LOOPBACK} ${SKIP_CUSTOM_RCLONE}
fi
...
id -u kasm &>/dev/null || adduser kasm

# TODO Prompt the user, or accept a flag for automation
rm -rf ${KASM_INSTALL_BASE}
...
openssl req -x509 -nodes -days 1825 -newkey rsa:2048 -keyout ${KASM_INSTALL_BASE}/certs/kasm_nginx.key -out ${KASM_INSTALL_BASE}/certs/kasm_nginx.crt -subj "/C=US/ST=VA/L=None/O=None/OU=DoFu/CN=$(hostname)/emailAddress=none@none.none" 2> /dev/null
...
then
    ...
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("proxy")).proxy.image) = "kasmweb/nginx:'${ROLLING_NGINX_TAG}'"' {} \;
fi

if [ "${USE_SLIM}" == "true" ];
then
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("kasm_guac")).kasm_guac.image) += "-alpine"' {} \;
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("kasm_api")).kasm_api.image) += "-alpine"' {} \;
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("kasm_manager")).kasm_manager.image) += "-alpine"' {} \;
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("kasm_agent")).kasm_agent.image) += "-alpine"' {} \;
    find ${KASM_INSTALL_BASE}/docker/.conf/*.yaml -exec ${SCRIPT_PATH}/bin/utils/yq_$(uname -m) -i '(.services | select(has("kasm_share")).kasm_share.image) += "-alpine"' {} \;
fi
